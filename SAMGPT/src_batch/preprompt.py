...
from models import DGI, GraphCL, Lp, GcnLayers, MLP, GatLayers, GraphSAGELayers, GINLayers
...
class PrePrompt(nn.Module):
    def __init__(self, n_in, n_h, activation, num_pretrain_dataset_num, num_layers_num, 
        ...
        if backbone == 'gat':
            ...
            self.gcn = GINLayers(n_in, n_h, num_layers_num, dropout)
        elif backbone == 'graphsage':
            self.gcn = GraphSAGELayers(n_in, n_h, num_layers_num, dropout)
    ...
    def compute_prelogits_LP(self, fea_pretext, str_layers, seq, adj, 
        sparse = False):
        if self.ablation_choice == 'None':
            return self.lp(self.gcn, seq, adj, sparse)
        else:
            fea_prelogits = self.lp(self.gcn, fea_pretext(seq) , adj, sparse) 
            str_prelogits = self.lp(self.gcn, seq, adj, sparse, str_layers)
            return self.ablation(fea_prelogits, str_prelogits)
        
    def compute_prelogits_GRAPHCL(self, fea_pretext, str_layers, seq, adj,
    ...
    def forward(self, seq_list, adj_list, sparse, msk, 
        samp_bias1, samp_bias2, lbl, samples = None, dataset_id = 0):        
        total_loss = torch.tensor(0.0, dtype=torch.float32).to(seq_list[0].device)
        dataset_id = int(dataset_id)
        if samples == None:
            logits = self.compute_prelogits_GRAPHCL(
                self.feature_prompt_layers[dataset_id], 
                self.structure_prompt_layers[dataset_id],
                seq_list, 
                adj_list,  
                sparse, msk, samp_bias1, samp_bias2)
            total_loss = self.loss(logits, lbl)
        else:
